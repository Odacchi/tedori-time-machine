...
- 型の役割: `TaxInput` は入力パラメータ、`TaxResult` は結果、`TaxBreakdown` は税・社保内訳。`TaxRuleSet` が各年のルール（基礎控除、配偶者控除とその所得制限、扶養控除、社会保険料率（本人/会社・40歳以上なら介護保険を上乗せ）、住民税率、給与所得控除テーブル、所得税率表、復興特別所得税率など）をまとめています。
...
- `src/domain/tax/calculator.ts` で以下の順序で計算:
  1. 社会保険料（本人・会社）を年収 × 料率で概算し、対象年に長期介護保険がある場合は40歳以上ならその料率を上乗せ
  2. `salaryIncomeDeductionTable` から給与所得控除額を決定（定額/「定率＋控除額」/「定率−控除額」の段階テーブルで計算し、控除額は年収を超えないよう上限をかける）
  3. 給与所得 − 所得控除（基礎控除・社会保険料控除・配偶者控除※給与所得が一定額を超える年は適用除外・扶養控除など）＝ 課税所得
...
- 給与所得控除は当時の定率/定額＋控除額の段階を `flat` `ratePlus` `rateMinus` で近似し、控除額が給与所得を超えないよう上限を設定。所得控除・扶養控除も簡略化テーブルで概算。
